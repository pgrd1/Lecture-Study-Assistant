...
import {
  ...
  parseOpenAiResponse,
  sanitizeOpenAiApiFailure,
  snapshotOpenAiExecuteRequest,
} from './openAiApiProtocol';
import { normalizeOpenAiOptionalNulls } from './openAiStrictSchema';
...
export type OpenAiApiAdapterOptions = Readonly<{
...
type ResponseOperation<Output extends JsonValue> = Readonly<{
  modelId: string;
...
class OpenAiApiAdapter implements AiProviderAdapter<'openai_api'> {
  ...
  async inspect(operation: ProviderConnectionOperation): Promise<ProviderInspection<'openai_api'>> {
    ...
    try {
      ...
      throw sanitizeOpenAiApiFailure(error);
    } finally {
    ...
    try {
      ...
      response = await this.#options.httpClient.requestJson(
        Object.freeze({
          ...
          endpoint: Object.freeze({ id: 'openai_models' as const }),
          authValue,
      ...
      this.#assertActive(active);
      return parseOpenAiModelList(response);
    } catch (error) {
      throw sanitizeOpenAiApiFailure(error);
    } finally {
      response = undefined;
  ...
  async probe(
    ...
    let active: ActiveOperation | undefined;
    try {
      const selectedModelId = assertOpenAiApiModelId(modelId);
      active = this.#beginOperation(operation.requestId, operation.signal);
      ...
      const result = await this.#executeResponse(
        Object.freeze({
          ...
          outputSchemaId: 'provider_probe_v1' as const,
      ...
      this.#assertActive(active);
      const finishedAt = this.#safeMilliseconds();
      return Object.freeze({
        status: 'ready' as const,
        reportedModelId: result.reportedModelId,
        latencyMs: Math.max(0, Math.trunc(finishedAt - startedAt)),
  ...
  async execute<Output extends JsonValue>(
    ...
    let active: ActiveOperation | undefined;
    try {
      const snapshot = snapshotOpenAiExecuteRequest(request);
      ...
      return await this.#executeResponse(
        Object.freeze({
  ...
  async #executeResponse<Output extends JsonValue>(
    operation: ResponseOperation<Output>,
    ...
    try {
      ...
      const outbound: ProviderHttpRequest = Object.freeze({
        ...
        signal: active.controller.signal,
        classifyError: classifyOpenAiApiError,
      });
      ...
      const parsed = parseOpenAiResponse(
        ...
        (value) =>
          operation.parseOutput(
            normalizeOpenAiOptionalNulls(value as JsonValue, operation.outputJsonSchema),
          ),
        this.#safeNow(),
      );
      this.#assertActive(active);
  ...
  async #readSecret(active: ActiveOperation): Promise<string> {
    ...
    if (typeof authValue !== 'string' || authValue.length === 0) {
      throw openAiApiError('PROVIDER_AUTH_REQUIRED');
    }
    return authValue;
  }
  ...
  #beginOperation(requestId: string, sourceSignal: AbortSignal): ActiveOperation {
    ...
    if (sourceSignal.aborted) throw openAiApiError('PROVIDER_CANCELLED');
    ...
    if (sourceSignal.aborted) {
      ...
      throw openAiApiError('PROVIDER_CANCELLED');
    }
    this.#assertActive(active);
    return active;
  }
  ...
  #assertActive(active: ActiveOperation): void {
    ...
    if (active.controller.signal.aborted) throw openAiApiError('PROVIDER_CANCELLED');
  }
  ...
  #safeNow(): string {
    ...
    if (typeof value !== 'string') throw openAiApiError('PROVIDER_OUTPUT_INVALID');
    return value;
  }

  #safeMilliseconds(): number {
    const value = this.#options.nowMilliseconds();
    if (!Number.isFinite(value) || value < 0) throw openAiApiError('PROVIDER_EXECUTION_FAILED');
    return value;
  }
}

export const createOpenAiApiAdapterForTest = (
  options: OpenAiApiAdapterOptions,
): AiProviderAdapter<'openai_api'> => new OpenAiApiAdapter(options);

export const createOpenAiApiAdapter = createOpenAiApiAdapterForTest;
